...
const displayImages = (images) => {
    ...
    if (images.length === 0) {
        ...
        notiflix.Notify.failure("На жаль, не знайдено зображень, що відповідають вашому запиту. Спробуйте ще раз.");
    } else {
...
const performSearch = async (searchQuery, page) => {
    ...
    if (data) {
        ...
        if (data.totalHits <= page * data.hits.length) {
            loadMoreButton.style.display = 'none';
            notiflix.Notify.warning("На жаль, ви дійшли до кінця результатів пошуку.");
        }
    }
};